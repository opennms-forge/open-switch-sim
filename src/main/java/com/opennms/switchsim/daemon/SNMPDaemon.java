...
import com.opennms.switchsim.SwitchSimController;

public class SNMPDaemon extends BaseAgent implements Runnable{
	
	private static final String PROPERTY_SLEEP_ON_CREATE = "mockSnmpAgent.sleepOnCreate";
	private static final Logger LOG = LoggerFactory.getLogger(SNMPDaemon.class);
	private static SNMPDaemon runningAgent;
    ...
    public SNMPDaemon(final File confFile, final URL moFile) {
     ...
     */
    public SNMPDaemon(final File confFile, final URL moFile, final String bindAddress) {
    ...
    public static SNMPDaemon createAgentAndRun(URL moFile, String bindAddress) throws InterruptedException {
        ...
        final SNMPDaemon agent = new SNMPDaemon(new File("/dev/null"), moFile, bindAddress);
    ...
    public static void exec(String[] args)  throws UnknownHostException, MalformedURLException, IOException  {
        ...
        try {
        	runningAgent = SNMPDaemon.createAgentAndRun(agentConfig.getMoFile(), listenSpec);
        } catch (InterruptedException e) {
            System.exit(0);
        }
    }

    public static SNMPDaemon getRunningAgent() {
    	//TODO Put check if agent is running and run loop if not
    	return runningAgent;
    }
    
    public static AgentConfigData parseCli(String[] args) throws UnknownHostException, MalformedURLException, IOException  {
    ...
    public ManagedObject findMOForOid(OID oid) {
